...
LOG_PATH = "live_detections.json"

SCENARIO_LABEL = "bright_indoor_live"
...
LOW_CONF_WARN_THRESH = 0.45
HIGH_UNKNOWN_RATIO = 0.30
...
def build_entity_timelines(log):
    ...
    gap_threshold = (FACE_EVERY_N + 1) / fps
...
def analyse_quality(entities, log, scenario_label):
    ...
    if global_avg_conf < LOW_CONF_WARN_THRESH:
        ...
        suggestions.append(
            ...
            "motion blur or camera defocus. Try increasing scene brightness or "
    ...
    if unknown_ratio > HIGH_UNKNOWN_RATIO:
        ...
        suggestions.append(
            ...
            "partial occlusion, or crop resolution too small for HOG model. "
            "Try more diverse face angles in training data."
        )
    ...
    if flicker_entities:
        ...
        suggestions.append(
            "Confidence flickering suggests intermittent occlusion or excessively "
            "lenient confidence threshold. Try increasing confidence threshold."
        )
...
print(f"[INFO] Camera: {width}x{height} @ {fps_input:.1f}fps")
...
while True:
    if time.time() - capture_start >= CAPTURE_DURATION_S:
        print(f"[INFO] {CAPTURE_DURATION_S}s capture complete — stopping.")
        break
    ...
    print(f"[INFO] FPS: {fps_v:.1f}  INF: {infer_ms:.1f}ms  "
          f"Frame: {total_frames}  Elapsed: {elapsed:.1f}/{CAPTURE_DURATION_S}s")

    cv2.putText(frame, f"FPS: {fps_v:.1f}  INF: {infer_ms:.1f}ms  {elapsed:.0f}/{CAPTURE_DURATION_S}s",
...
with open("live_fps_tracker.txt", "w") as f:
